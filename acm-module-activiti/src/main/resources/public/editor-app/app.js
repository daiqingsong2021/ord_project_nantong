 ...
 * 修改了fetchModel方法中可以根据发布ID，流程定义ID和流程实例ID获取流程
 */
'use strict';
...
activitiModeler
  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
		      ...
		      $rootScope.modeReadonly = true; //模块只读
            ...
            function fetchModel(modelId, deployId, procDefId, procInstId) {

                var modelUrl = "";
                if(procInstId){
                    modelUrl = KISBPM.URL.getProcessInstance(procInstId);
                }else if(deployId){
                    modelUrl = KISBPM.URL.getDeployment(deployId);
                }else if(procDefId){
                    modelUrl = KISBPM.URL.getProcessDefinition(procDefId);
                }else{
                    modelUrl = KISBPM.URL.getModel(modelId);
                    $rootScope.modeReadonly = false;
                    ORYX.CONFIG.MODE_READONLY = "write";
                }
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
                    ...
                    var deployId = EDITOR.UTIL.getParameterByName('deployId');
                    var procDefId = EDITOR.UTIL.getParameterByName('procDefId');
                    var procInstId = EDITOR.UTIL.getParameterByName('procInstId');

	                fetchModel(modelId, deployId, procDefId, procInstId);
