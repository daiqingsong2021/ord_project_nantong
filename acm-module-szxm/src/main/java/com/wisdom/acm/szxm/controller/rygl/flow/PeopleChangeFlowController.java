...
public class PeopleChangeFlowController extends WFController
{
    @Autowired
    ...
    public ApiResult startWorkFlowAfter(@RequestBody WfRuningProcessForm form) {
        ...
        peopleChangeService.updateSelectiveByIds(updatePo,ids);
    ...
    public ApiResult completeWorkFlowAfter(@RequestBody WfRuningProcessForm form) {
        ...
        peopleChangeService.approvePeopleChangeFlow(fromData.get(0).getBizType(), ids);
        return ApiResult.success();
    }
        ...
        if (!ObjectUtils.isEmpty(form.getCandidate()) && !ObjectUtils.isEmpty(form.getCandidate().getActivities())) {
            ...
            if (!ObjectUtils.isEmpty(codeList)) {
                if (codeList.contains("section") || codeList.contains("project")) {
                    ...
                    List<PeopleChangePo> peopleChangePos=peopleChangeService.selectByIds(ids);
                    if (!ObjectUtils.isEmpty(peopleChangePos)) {
                        //根据code选择传入标段id 或项目id
                        sectionId = peopleChangePos.get(0).getSectionId();
                        projectId = peopleChangePos.get(0).getProjectId();
                    }
                }
