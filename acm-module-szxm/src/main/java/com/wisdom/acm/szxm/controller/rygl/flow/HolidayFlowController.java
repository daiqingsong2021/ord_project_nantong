...
@RestController
@RequestMapping("rygl/attenter/holiday")
public class HolidayFlowController  extends WFController
{
    ...
    public ApiResult startWorkFlowAfter(@RequestBody WfRuningProcessForm form) {
        ...
        holidayService.updateSelectiveByIds(updatePo,ids);
    ...
    public ApiResult completeWorkFlowAfter(@RequestBody WfRuningProcessForm form) {
        ...
        holidayService.approveHolidayFlow(fromData.get(0).getBizType(), ids);
        return ApiResult.success();
    }
        ...
        if (!ObjectUtils.isEmpty(form.getCandidate()) && !ObjectUtils.isEmpty(form.getCandidate().getActivities())) {
            ...
            if (!ObjectUtils.isEmpty(codeList)) {
                if (codeList.contains("section") || codeList.contains("project")) {
                    ...
                    List<HolidayPo> holidayPos=holidayService.selectByIds(ids);
                    if (!ObjectUtils.isEmpty(holidayPos)) {
                        //根据code选择传入标段id 或项目id
                        sectionId = holidayPos.get(0).getSectionId();
                        projectId = holidayPos.get(0).getProjectId();
                    }
                }
