...
import com.wisdom.acm.processing.common.UUIDHexGenerator;
                ...
                if (!ObjectUtils.isEmpty(value))
                {
                    ...
                    for (int i = startIndex + 1; i < endIndex; i++)
                    {
                        ...
                        if (value instanceof String)
                        {
                            ...
                            //设置第一个1号线数据字体是24号
                            if("1".equals(value) && 3==i && 4==endIndex && startEndIndex.size()==1 && seMap.size()==3){
                                insertRun.setFontSize(24);
                            }
                            if("3".equals(value) && 3==i && 4==endIndex && startEndIndex.size()==1 && seMap.size()==3){
                                insertRun.setFontSize(24);
                            }
//                            insertRun.setText(String.valueOf(value));
                            if (String.valueOf(value).contains("\n")) {
                                String[] lines = String.valueOf(value).split("\n");
                                insertRun.setText(lines[0], 0); // set first line into XWPFRun
                                for(int q=1;q<lines.length;q++){
                                    // add break and insert new text
                                    insertRun.addBreak();
                                    insertRun.setText(lines[q]);
                                }
                            } else {
                                insertRun.setText(String.valueOf(value), 0);
                            }
    ...
    public void generateWordZll(InputStream sourceFileInputStream, Map<String, Object> params,
                                String destFile, List<String> keyNoContains) throws Exception {
        MyXWPFDocument doc = new MyXWPFDocument(sourceFileInputStream);
        this.replaceInPara(doc, params); // 替换文本里面的变量
        this.replaceInTable(doc, params); // 替换表格里面的变量
        FileOutputStream os = new FileOutputStream(destFile);
        ...
        for(String i : keyNoContains){
            deleteTable(tables.get(Integer.parseInt(i)));
        }
        String test="";
        //tableTextStyle(tableCellsLast,0,test);
        //deleteParagraph(doc);
        doc.write(os);
        this.close(os);
        this.close(sourceFileInputStream);
    ...
    {
        if("注：本表由承包单位编制，一式六份，建设单位二份、项目监理机构一份、施工单位三份。".equals(text)){
            XWPFParagraph p0 = tableCell.addParagraph();
            p0.setAlignment( ParagraphAlignment.LEFT);//对齐方式
            ...
            r0.setFontSize(10);
            // 设置使用何种字体
            r0.setFontFamily( "宋体 (中文标题)" );
            // 设置上下两行之间的间距
            r0.setTextPosition(5);
            // r0.setColor( "333333" );
            r0.setText(text);
        }
    }
...
//     * @param xwpf
//     */
//    public void deleteParagraph(XWPFDocument xwpf){
//        List<XWPFParagraph>itPg=xwpf.getParagraphs();
//        for(int i=0;i<itPg.size();i++){
//            if(itPg.get(i).getRuns().size()==0){
//                if(i==0){
//                    break;
//                }
//                xwpf.removeBodyElement(0);
//            }
//        }
//    }
}
