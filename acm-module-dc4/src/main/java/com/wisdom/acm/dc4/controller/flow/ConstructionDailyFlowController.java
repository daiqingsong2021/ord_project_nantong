package com.wisdom.acm.dc4.controller.flow;

import com.google.common.collect.Lists;
import com.wisdom.acm.dc4.common.SzxmCommonUtil;
import com.wisdom.acm.dc4.common.SzxmEnumsUtil;
...
@RequestMapping("sgrk/constructionDaily")
 public class ConstructionDailyFlowController extends WFController
{
    @Autowired
        ...
        if (!ObjectUtils.isEmpty(form.getCandidate()) && !ObjectUtils.isEmpty(form.getCandidate().getActivities())) {
            ...
            if (!ObjectUtils.isEmpty(codeList)) {
                if (codeList.contains("section") || codeList.contains("project")) {
                    ...
                    List<ConstructionDailyPo> constructionPos = constructionDailyService.selectByIds(ids);
                }
    ...
    public ApiResult startWorkFlowAfter(@RequestBody WfRuningProcessForm form) {
        ...
        updatePo.setReviewStatus(SzxmEnumsUtil.StatusEnum.APPROVAL.getCode());
        //获取当前登录人信息
        UserInfo userInfo= commUserService.getLoginUser();
        updatePo.setReviewer(userInfo.getId());
        updatePo.setReviewTime(new Date());
        List<Integer> ids= Lists.newArrayList();
        for(String bizId:bizIds)
        {
            ids.add(Integer.valueOf(bizId));
        }
        constructionDailyService.updateSelectiveByIds(updatePo,ids);
    ...
    public ApiResult completeWorkFlowAfter(@RequestBody WfRuningProcessForm form) {
        ...
        constructionDailyService.approveConstructionDailyWorkFlow(fromData.get(0).getBizType(), ids);
        return ApiResult.success();
    }
    ...
    public ApiResult backActivityAfter(@RequestBody WfRuningProcessForm form) {
        ...
        List<String> bizIds = ListUtil.toValueList(fromData,"bizId",String.class);
        ConstructionDailyPo updatePo =new ConstructionDailyPo();
        updatePo.setReviewStatus(SzxmEnumsUtil.StatusEnum.REJECT.getCode());
        List<Integer> ids=Lists.newArrayList();
        for(String bizId:bizIds)
        {
            ids.add(Integer.valueOf(bizId));
    ...
    public ApiResult terminateWorkFlowAfter(@RequestBody WfRuningProcessForm form)
    {
        ...
        List<String> bizIds = ListUtil.toValueList(fromData,"bizId",String.class);
        ConstructionDailyPo updatePo =new ConstructionDailyPo();
        updatePo.setReviewStatus(SzxmEnumsUtil.StatusEnum.INIT.getCode());
        List<Integer> ids=Lists.newArrayList();
        for(String bizId:bizIds)
        {
            ids.add(Integer.valueOf(bizId));
