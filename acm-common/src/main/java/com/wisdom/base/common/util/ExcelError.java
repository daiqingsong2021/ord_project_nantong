...
public class ExcelError implements Serializable {
	...
	private Map<String,Map<String,List<Map<String,String>>>> errorInfo = new HashMap<String,Map<String,List<Map<String,String>>>>();
	// 行下表
	...
	{
		addError(FormatUtil.toString(cellIndex),cellName,error);
	}
	...
	public void addError(String cellIndex,String cellName,String error)
	{
		...
		errorMap.put("cellIndex", FormatUtil.toString(cellIndex));
	...
	public List<Map<String,Object>> getErrorList()
	{
		...
		while(sheetIter.hasNext())
		{
			...
			while(rowIter.hasNext())
			{
				...
				{
					error+=ObjectUtils.isEmpty(error) ? map.get("cellName")+"列"+map.get("error")
	...
	private List<Map<String,Object>> sort(List<Map<String,Object>> list)
	{
		for(int i=0;i<list.size()-1;i++)
		{
			...
			{
				int sheetnum1 = FormatUtil.parseInt(list.get(j).get("sheetnum"));
				int row1 = FormatUtil.parseInt(list.get(j).get("row"));
				int sheetnum2 = FormatUtil.parseInt(list.get(j+1).get("sheetnum"));
				int row2 = FormatUtil.parseInt(list.get(j+1).get("row"));

				Map<String,Object> map;
				if(sheetnum1 > sheetnum2
						|| (sheetnum1 == sheetnum2 && row1 > row2))
				{
					map = list.get(j);
	...
	public Map<String,Map<String,List<Map<String,String>>>> getErrorInfo(){
