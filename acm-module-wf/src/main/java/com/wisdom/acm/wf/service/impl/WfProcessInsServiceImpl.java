...
public class WfProcessInsServiceImpl implements WfProcessInsService {
    ...
    public WfRunProcessVo startProcess(WfStartProcessForm form) {
        ...
        if (!ObjectUtils.isEmpty(bizType)) {
            ...
            //和品高对接，做流程推送。
            form.setTitle(BaseUtil.getLoginUser().getName() + "发起工作流,工作流名称为【" + form.getTitle() + "】");
            form.setStatus("start");
            form.setMsgStatus("todo");
            form.setBusinessStatus("executing");
            actSsoService.insertTodoBusiness(form, procVo);
            actSsoService.insertTodoMsg(form, procVo);
